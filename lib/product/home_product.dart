...
class HomeProductBlock implements Disposable {
  ...
  final StreamController<List<Product>> _productController =
      StreamController<List<Product>>.broadcast(); // enter point
  final StreamController<int> _categoryController =
      StreamController<int>.broadcast(); // first enter point category_id

  Stream<List<Product>> get productsStrem => _productController
      .stream; // output list of products belong to category_id

  StreamSink<int> get fetchProducts =>
      _categoryController.sink; // input category id   // نقطة الادخال
